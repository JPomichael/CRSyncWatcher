...
namespace CRS.Sync.Watcher.ConsoleApplication.Base
{
    ...
    public class Base
    {
        ...
        //!  通用代码分类名数组
        public readonly string[] publiCodeCategoryList = new string[] { "state", "city", "hotelsta" }; //x 
        //x "country", "state", "city", "cityarea", "hotelsta", "hotelcat", "hotelstt" 

        public void SyncService(string staticFolderSavePath)
        {
            //!   循环数组内分类名
            for (int i = 0; i < publiCodeCategoryList.Length; i++)
            {
                ...
                if (dcPublicResult.result == 0 && dcPublicResult.dcPublics != null)
                {
                    ...
                    {
                        case "state":
                            ProvinceSync(_dcPublicList);
                            break;
                        case "city":
                            CitySync(_dcPublicList);
                            break;
                        case "hotelsta":
                            StarSync(_dcPublicList);
                            break;
                    }
                    ...
                    Console.WriteLine("CRS接口GetPublicInterfaceByCategory返回为NULL!");
            }
        }
        ...
        public void ProvinceSync(List<CRS.Sync.Watcher.Service.WCFMobileServer.DcPublic> _dcPublicList)
        {
            ...
            foreach (var _provinceList in provinceList)
            {
                if (!_provinceService.CheckIsAny(_provinceList))
                {
                    ...
                    if (!result)
                    {
                        ...
                        Console.WriteLine("录入数据失败： " + _provinceList.province_name + " " + _provinceList.code);
                    }
                }
                else
                {
        ...
        /// </summary>
        /// <param name="_dcPublicList"></param>
        public void CitySync(List<CRS.Sync.Watcher.Service.WCFMobileServer.DcPublic> _dcPublicList)
        {
            ...
            foreach (var _cityList in cityList)
            {
                if (!_cityService.CheckIsAny(_cityList))
                {
                    ...
                    if (!result)
                    {
                        Console.WriteLine("录入数据失败： " + _cityList.city_name + " " + _cityList.code);
                    }
                }
                else
                {
        ...
        /// </summary>
        /// <param name="_dcPublicList"></param>
        public void StarSync(List<CRS.Sync.Watcher.Service.WCFMobileServer.DcPublic> _dcPublicList)
        {
            ...
            foreach (var _starList in starList)
            {
                if (!_starService.CheckIsAny(_starList))
                {
                    ...
                    if (!result)
                    {
                        Console.WriteLine("录入数据失败： " + _starList.cname + " " + _starList.id);
                    }
                }
                else
                {
